...
export function useContentFactory() {
  ...
  const transformContentData = useCallback(
    async (
      contentIds: bigint[],
      ...
      for (let i = 0; i < contentIds.length; i++) {
        ...
        try {
          ...
          let description = `Content created by ${creator.slice(
            0,
            6
          )}...${creator.slice(-4)}`;
          ...
          console.error(
            `Error fetching detailed info for content ${contentId}:`,
            error
          );
          ...
          const fallbackContent: ContentData = {
            ...
            description: `Content created by ${creator.slice(
              0,
              6
            )}...${creator.slice(-4)}`,
  ...
  const fetchContents = useCallback(async () => {
    ...
    try {
      ...
      if (Array.isArray(contentsData) && contentsData.length >= 3) {
        ...
        const transformedContents = await transformContentData(
          contentIds,
          contractAddresses,
          creators
        );
        console.log(transformedContents, "tx");
